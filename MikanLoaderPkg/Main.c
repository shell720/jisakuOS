...
EFI_STATUS OpenBlockIoProtocolForLoaderImage(EFI_HANDLE image_handle, EFI_BLOCK_IO_PROTOCOL** block_io){
    EFI_STATUS status;
    EFI_LOADED_IMAGE_PROTOCOL* loaded_image;
    ...
    status = gBS->OpenProtocol(loaded_image->DeviceHandle, &gEfiBlockIoProtocolGuid, (VOID**)block_io, image_handle, NULL, EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
...
EFI_STATUS EFIAPI UefiMain( EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *system_table){
    ...
    status = root_dir->Open(root_dir, &kernel_file, L"\\kernel.elf", EFI_FILE_MODE_READ, 0);//kennel.elfを読み込み専用で開く
    if (EFI_ERROR(status)){
    ...
    // カーネルを最終的に読み込むメモリ領域を割り当てる
    Elf64_Ehdr* kernel_ehdr = (Elf64_Ehdr*)kernel_buffer;
    UINT64 kernel_first_addr, kernel_last_addr;
    CalcLoadAddressRange(kernel_ehdr, &kernel_first_addr, &kernel_last_addr);
    UINTN num_pages = (kernel_last_addr - kernel_first_addr + 0xffff)/0x1000;
    ...
    if (status == EFI_SUCCESS){
        ...
        status = OpenBlockIoProtocolForLoaderImage(image_handle, &block_io);
        if (EFI_ERROR(status)){
