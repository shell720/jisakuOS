...
Terminal::Terminal(Task& task, const TerminalDescriptor* term_desc): task_{task}{
    ...
    if (show_window_){
        window_ = std::make_shared<ToplevelWindow>(
            kColumns*8 + 8 + ToplevelWindow::kMarginX, kRows*16 + 8 + ToplevelWindow::kMarginY, screen_config.pixel_format, "Terminal");
...
void Terminal::Scroll1(){
    Rectangle<int> move_src{ToplevelWindow::kTopLeftMargin + Vector2D<int>{4, 4 + 16}, {8*kColumns, 16*(kRows-1)}};
...
void Terminal::ExecuteLine(){
    ...
    if (pipe_char){
        *pipe_char = 0;
        ...
        auto term_desc = new TerminalDescriptor{subcommand, ture, false, {pipe_fd, files_[1], files_[2]}};
        files_[1] = pipe_fd;
    ...
    if (strcmp(command, "echo") == 0 ){
        ...
        for (int i=0; i<pci::num_devices; ++i){
            ...
            PrintToFD(*files_[1], "%02x: %02x.%d vend= %04x head= 02%x class= %02x.%02x.%02x\n", dev.bus, dev.device, dev.function,
                vendor_id, dev.header_type, dev.class_code.base, dev.class_code.sub, dev.class_code.interface);
        }
...
void Terminal::Print(const char* s, std::optional<size_t> len){
    ...
    Vector2D<int> draw_pos{ToplevelWindow::kTopLeftMargin.x, cursor_before.x};
    ...
    while (true){
        ...
        switch (msg->type){
            case Message::kTimerTimeout:
                ...
                if (window_isactive && show_window){
