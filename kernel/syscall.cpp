...
namespace syscall{
    ...
    #define SYSCALL(name) Result name(uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5, uint64_t arg6)
    ...
    SYSCALL(OpenWindow){
        ...
        __asm__("cli");
        const auto layer_id = layer_manager->NewLayer().SetWindow(win).SetDraggable(true).Move({x,y}).ID();
